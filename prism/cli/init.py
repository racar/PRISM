...
@click.option("--no-speckit", is_flag=True, help="Skip Spec-Kit initialization")
@click.option("--no-embeddings", is_flag=True, help="Use FTS5 only (available in Fase 1)")
def init(name: str | None, no_speckit: bool, no_embeddings: bool) -> None:
    """Initialize a new PRISM project."""
    project_dir = Path.cwd() / name if name else Path.cwd()
    init_project(project_dir, skip_speckit=no_speckit)
